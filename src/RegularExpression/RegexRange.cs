...
public class RegexRange<T>(RegexProvider<T> provider, T? minimum, T? maximum, bool canTakeMinimum = true, bool canTakeMaximum = true) : RegexObject<T>(provider)
{
    /// <summary>
    /// 内部范围的最小值。
    /// </summary>
    protected T? minimum = minimum;
    /// <summary>
    /// 内部范围的最大值。
    /// </summary>
    protected T? maximum = maximum;

    /// <summary>
    /// 一个值，指示内部范围是否取到最小值。
    /// </summary>
    protected bool canTakeMinimum = canTakeMinimum;
    /// <summary>
    /// 一个值，指示内部范围是否取到最大值。
    /// </summary>
    protected bool canTakeMaximum = canTakeMaximum;
